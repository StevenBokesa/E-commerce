							...
							<div class="main-posts">											
								<div class="activity-posts">																					
									<div class="activity-group1">																					
										<div class="main-user-dts1">																					
											...
											<div class="user-text3">
												<c:choose>
														    <c:when test="${post.user_id==sessionScope.user_id}">
														        <h4>Me</h4>
														        <span>posted an update at <c:out value="${post.post_time}" /></span>
														    </c:when>    
														    <c:otherwise>
									...
									<div class="activity-post-reply">
											...
											<form action="commentPage" method="post" id="post">
											    <% Post postg = (Post)request.getAttribute("post"); %>
												<input type="hidden" name="post_id" value="<%=postg.getPost_id()%>">
												<input class="areply-post" type="text"  name="comment" placeholder="Write a reply">
												<button class="areply-post-btn" type="submit">Reply</button>
											</form>
									</div>
									<c:forEach var="comment" items="${comment}" varStatus="loopStatus">
									<div class="activity-reply">
										<div class="activity-group1">																					
											<div class="main-user-dts1">																					
												...
												<div class="user-text3">
													<c:choose>
														    <c:when test="${comment.user_id==sessionScope.user_id}">
														        <h4>Me</h4>
														        <span>posted an update at <c:out value="${comment.comment_time}" /></span>
														    </c:when>    
														    <c:otherwise>
															   <% Comment comment = (Comment)pageContext.getAttribute("comment"); %>
 															   <h4> <%= new UserDAO().getUserById(comment.getUser_id()).getFirst_name() %> </h4>
														       <span>posted an update at <c:out value="${comment.comment_time}" /></span>
														    </c:otherwise>
													</c:choose>
												</div>											
